...
try:
    from .models.terrain_cnn import (TerrainCNN, MobileNetV2Classifier,
                                     load_model)
except ImportError:
    # Fallback for when running as script
    from terrain_locomotion.perception.models.terrain_cnn import (
        TerrainCNN, MobileNetV2Classifier, load_model)
...
class TerrainClassifierNode(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter(
            'model_type',
            'custom')  # 'custom' or 'mobilenet'
        # Use feature-based if no model
        self.declare_parameter('use_demo_mode', True)
        ...
        if model_path and os.path.exists(model_path):
            try:
                ...
                self.get_logger().info(
                    f'Loaded {model_type} model from {model_path}')
            ...
            if model_type == 'mobilenet':
                self.model = MobileNetV2Classifier(
                    num_classes=len(self.classes), pretrained=False)
            else:
            ...
            if use_demo_mode:
                self.get_logger().info(
                    'Running in DEMO MODE with feature-based classification')
            else:
                self.get_logger().warn(
                    'No model provided, using random weights')
                self.use_cnn = True
    ...
    def image_callback(self, msg):
        try:
            ...
            annotated_image = self.create_annotated_image(
                cv_image, terrain_class, confidence)
            ...
            if confidence < confidence_threshold:
                self.get_logger().warn(
                    f'Low confidence classification: {terrain_class} ({
                        confidence:.2f})')
            else:
                self.get_logger().info(
                    f'Terrain: {terrain_class} (confidence: {
                        confidence:.2f})')
    ...
    def classify_terrain(self, cv_image):
        ...
        try:
            ...
            self.get_logger().error(
                f'Error in terrain classification: {
                    str(e)}')
            return 'flat', 0.5
    ...
    def _classify_with_features(self, cv_image):
        """Classify using hand-crafted features (demo mode).

        This method analyzes image features like color, texture, and
        edges to classify terrain. It's designed to work without a
        trained model.
        """
        ...
        h, s, v = cv2.split(hsv)
        # saturation_mean = np.mean(s)  # For future use
        ...
        if vertical_edges > 1000 and horizontal_edges > 800:
            pattern_score = min(
                (vertical_edges + horizontal_edges) / 5000, 1.0)
    ...
    def create_annotated_image(self, image, terrain_class, confidence):
        ...
        cv2.rectangle(annotated, (10, 10), (text_width +
                      20, text_height + 20), color, -1)

        # Draw text
        cv2.putText(annotated, text, (15, text_height + 15),
                    font, font_scale, (255, 255, 255), thickness)

        # Add confidence bar
        bar_length = int(300 * confidence)
        cv2.rectangle(annotated, (10, text_height + 40),
                      (310, text_height + 60), (128, 128, 128), -1)
        cv2.rectangle(annotated, (10, text_height + 40),
                      (10 + bar_length, text_height + 60), color, -1)
