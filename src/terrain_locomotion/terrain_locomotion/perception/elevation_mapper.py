# File:
# ~/terrain_locomotion_ws/src/terrain_locomotion/terrain_locomotion/perception/elevation_mapper.py
...
class ElevationMapper(Node):
    def __init__(self):
        ...
        self.map_timer = self.create_timer(
            0.2, self.publish_elevation_map)  # 5Hz
    ...
    def update_elevation_map(self, depth_image, transform):
        ...
        for v in range(0, height, 4):  # Subsample for performance
            for u in range(0, width, 4):
                ...
                # Transform to world frame (simplified - assumes camera looking
                # down)
    ...
    def add_point_to_grid(self, x, y, z):
        ...
        if (0 <= grid_x < self.elevation_grid.shape[0] and
            ...
            if np.isnan(
                    self.elevation_grid[grid_x, grid_y]) or z > self.elevation_grid[grid_x, grid_y]:
    ...
    def publish_elevation_map(self):
        ...
        try:
            ...
            self.get_logger().error(
                f'Error publishing elevation map: {
                    str(e)}')
    ...
    def create_elevation_visualization(self):
        ...
        if not valid_mask.any():
            ...
            return np.zeros(
                (vis_grid.shape[0], vis_grid.shape[1], 3), dtype=np.uint8)
        ...
        if max_elevation > min_elevation:
            normalized = (vis_grid - min_elevation) / \
        ...
        colored_map[valid_mask, 0] = (
            normalized[valid_mask] * 255).astype(np.uint8)  # Red
        colored_map[valid_mask, 1] = (
            (1 - normalized[valid_mask]) * 128).astype(np.uint8)  # Green
        colored_map[valid_mask, 2] = (
            (1 - normalized[valid_mask]) * 255).astype(np.uint8)  # Blue
        ...
        cv2.circle(colored_map, (robot_grid_y, robot_grid_x),
                   5, (0, 255, 0), -1)
