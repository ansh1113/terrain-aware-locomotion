...
class SafetyMonitor(Node):
    ...
    def __init__(self):
        ...
        self.max_tilt_angle_rad = math.radians(
            self.get_parameter('max_tilt_angle').value)
        ...
        self.safety_timer = self.create_timer(
            1.0 / check_rate, self.safety_check_callback)

        self.get_logger().info('Safety Monitor initialized')
        self.get_logger().info(
            f'Max joint velocity: {
                self.max_joint_velocity} rad/s')
    ...
    def check_joint_position_limits(self):
        ...
        for joint_name, position in self.current_joint_positions.items():
            ...
            # Extract 'haa', 'hfe', or 'kfe'
            joint_type = joint_name.split('_')[-2]
            ...
            if joint_type in self.joint_position_limits:
                ...
                if position < min_limit or position > max_limit:
                    violation = f'{joint_name} position {
                        math.degrees(position):.1f}° outside limits [{
                        math.degrees(min_limit):.1f}°, {
                        math.degrees(max_limit):.1f}°]'
    ...
    def check_joint_velocity_limits(self):
        ...
        for joint_name, velocity in self.current_joint_velocities.items():
            if abs(velocity) > self.max_joint_velocity:
                violation = f'{joint_name} velocity {
                    velocity:.2f} rad/s exceeds limit {
                    self.max_joint_velocity:.2f} rad/s'
                self.violations.append(violation)
    ...
    def check_joint_effort_limits(self):
        ...
        for joint_name, effort in self.current_joint_efforts.items():
            if abs(effort) > self.max_joint_torque:
                violation = f'{joint_name} torque {
                    effort:.1f} Nm exceeds limit {
                    self.max_joint_torque:.1f} Nm'
                self.violations.append(violation)
    ...
    def check_robot_stability(self):
        ...
        if abs(roll) > self.max_tilt_angle_rad:
            violation = f'Roll angle {
                math.degrees(roll):.1f}° exceeds limit {
                math.degrees(
                    self.max_tilt_angle_rad):.1f}°'
            self.violations.append(violation)

        # Check pitch
        if abs(pitch) > self.max_tilt_angle_rad:
            violation = f'Pitch angle {
                math.degrees(pitch):.1f}° exceeds limit {
                math.degrees(
                    self.max_tilt_angle_rad):.1f}°'
            self.violations.append(violation)
