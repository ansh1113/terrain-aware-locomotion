...
class QuadrupedIK:
    ...
    def solve(
        ...
        # Check HAA limits
        if not (self.joint_limits['haa'][0] <= haa_angle <= self.joint_limits['haa'][1]):
            return None
        ...
        # Check KFE limits
        if not (self.joint_limits['kfe'][0] <= kfe_angle <= self.joint_limits['kfe'][1]):
            return None
        ...
        # Check HFE limits
        if not (self.joint_limits['hfe'][0] <= hfe_angle <= self.joint_limits['hfe'][1]):
            return None
...
class QuadrupedFullIK:
    ...
    def get_neutral_stance(self, height: float = -0.4) -> Dict[str, np.ndarray]:
...
def test_ik():
    ...
    for leg, angles in solutions.items():
        if angles:
            haa, hfe, kfe = angles
            print(f"  {leg:12s}: HAA={np.degrees(haa):6.2f}°, HFE={np.degrees(hfe):6.2f}°, KFE={np.degrees(kfe):6.2f}°")
        else:
