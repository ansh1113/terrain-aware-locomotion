# File:
# ~/terrain_locomotion_ws/src/terrain_locomotion/terrain_locomotion/control/gait_controller.py
...
class GaitControllerNode(Node):
    def __init__(self):
        ...
        self.control_timer = self.create_timer(
            0.01, self.control_callback)  # 100Hz
    ...
    def control_callback(self):
        ...
        try:
            with self.trajectory_lock:
                ...
                elapsed_time = (
                    current_time - self.trajectory_start_time).nanoseconds / 1e9
        ...
        for i, point in enumerate(self.current_trajectory.points):
            ...
            if elapsed_time <= point_time:
                if i == 0:
                    ...
                    alpha = (elapsed_time - prev_time) / \
                        (point_time - prev_time)
                    alpha = max(0.0, min(1.0, alpha))  # Clamp to [0,1]

                    interpolated_positions = []
                    for j in range(len(point.positions)):
                        pos = (
                            1.0 - alpha) * prev_point.positions[j] + alpha * point.positions[j]
    ...
    def apply_safety_limits(self, joint_commands):
        ...
        for i, command in enumerate(joint_commands):
            ...
            if abs(
                    self.current_joint_velocities[i]) > self.max_joint_velocity * 0.8:
            ...
            if abs(joint_pos) > math.pi:  # Basic joint limit
                if (joint_pos > 0 and limited_command > 0) or (
                        joint_pos < 0 and limited_command < 0):
