...
class GaitGenerator:
    ...
    def get_leg_states(self) -> Dict[str, Dict]:
        ...
        for leg in self.legs:
            ...
            if in_stance:
                ...
                phase_progress = (
                    (leg_phase - pattern['duty_factor']) /
                    (1.0 - pattern['duty_factor']))
    ...
    def get_swing_trajectory_point(
            self,
            leg_name: str,
            start_pos: np.ndarray,
            end_pos: np.ndarray,
            max_height: float = 0.05) -> np.ndarray:
    ...
    def select_gait_for_terrain(
            self,
            terrain_type: str,
            velocity: float = 0.0) -> GaitType:
...
def get_gait_frequency_for_terrain(
        terrain_type: str,
...
if __name__ == '__main__':
    ...
    for step in range(num_steps):
        ...
        for leg, state in states.items():
            ...
            print(
                f"  {
                    leg:12s}: {phase_str} (phase: {
                    state['phase']:.2f}, progress: {
                    state['progress']:.2f})")
        print()
    ...
    for terrain in terrains:
        for vel in velocities:
            ...
            print(f"{terrain:8s} @ {vel:.1f} m/s -> "
                  f"{gait.value:6s} ({freq:.1f} Hz)")
