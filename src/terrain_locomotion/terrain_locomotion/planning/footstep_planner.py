# File:
# ~/terrain_locomotion_ws/src/terrain_locomotion/terrain_locomotion/planning/footstep_planner.py
...
class FootstepPlannerNode(Node):
    ...
    def confidence_callback(self, msg):
        ...
        if self.terrain_confidence < threshold:
            self.get_logger().warn(
                f'Low terrain confidence: {
                    self.terrain_confidence:.2f}')
    ...
    def planning_callback(self):
        """Main planning loop"""
        if not self.is_planning and (
                abs(self.desired_velocity.linear.x) > 0.01 or
                abs(self.desired_velocity.linear.y) > 0.01 or
                abs(self.desired_velocity.angular.z) > 0.01):
            self.plan_footsteps()

    def plan_footsteps(self):
        """Generate footstep plan based on terrain and motion"""
        try:
            # Get terrain-adapted parameters
            terrain_config = self.terrain_params.get(
                self.current_terrain, self.terrain_params['flat'])
            ...
            if self.terrain_confidence < threshold:
                ...
                self.get_logger().info(
                    'Applying conservative gait due to low confidence')
    ...
    def generate_footstep_sequence(
        ...
        # angular_vel = self.desired_velocity.angular.z  # For future use
        ...
        for step_idx in range(num_steps):
            ...
            for foot_name in moving_feet:
                ...
                target_pos = base_pos + \
                    np.array([forward_offset, lateral_offset, 0])
    ...
    def publish_footstep_plan(self, footsteps):
        ...
        self.get_logger().info(
            f'Published {
                len(footsteps)} footsteps for terrain: {
